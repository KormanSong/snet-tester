...
PySide6 port of the Phase 2 integrated version.  Uses typed dataclass
events/commands, V2SerialWorker, and MockTransport.  All cross-package
imports converted to relative within snet_tester2.
"""
...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self, mock_mode: bool = False, port: str = 'COM6', baud: int = 115200):
        super().__init__()
        self._port = port
        self._baud = baud
        ...
        self._latencies: Optional[list] = [] if not self._worker_config.run_forever else None
        self._rx_ratios: Optional[list] = [] if not self._worker_config.run_forever else None
        self._total = 0
        ...
        if mock_mode:
            ...
            worker_config = WorkerConfig(
                rx_timeout_s=self._worker_config.rx_timeout_s,
                sample_period_s=max(0.001, self._worker_config.sample_period_s),
                run_forever=self._worker_config.run_forever,
                test_count=self._worker_config.test_count,
            )
            ...
            if self._port_combo is not None and self._port:
    ...
    def _init_relay_channel_selector(self):
        ...
        self._relay_channel_group.idClicked.connect(self._on_relay_channel_changed)
    ...
    def _handle_event(self, event):
        ...
        if isinstance(event, SampleReceivedEvent):
            ...
            print(format_sample_log(sample, self._worker_config.run_forever, self._worker_config.test_count))
    ...
    def _on_ad_command_toggled(self, checked: bool):
        if self._worker is None:
            return
        self._command_queue.put(WriteVarCommand(var_index=VarIndex.READ_AD_FLAG, value=1 if checked else 0))
    ...
    def _on_mode_toggled(self, checked: bool):
        ...
        if self._worker is None:
            return
        self._command_queue.put(WriteVarCommand(var_index=VarIndex.MODE_FLAG, value=1 if checked else 0))
    ...
    def _on_port_selected(self, port_name: str):
        ...
        transport = SerialTransport(port=port_name, baud=self._baud)
        worker_config = WorkerConfig(
            rx_timeout_s=self._worker_config.rx_timeout_s,
            sample_period_s=self._worker_config.sample_period_s,
            run_forever=self._worker_config.run_forever,
            test_count=self._worker_config.test_count,
        )
        self._worker = V2SerialWorker(
    ...
    def print_summary(self):
        ...
        if self._worker_config.run_forever and self._total == 0:
            return
