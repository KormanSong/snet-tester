"""Tests for v2 frame encoding/decoding and payload serialization.

Mirrors tests/test_codec.py but imports from snet_tester2.
Key changes vs v1:
  - SnetCommand enum replaces REQUEST_CMD / RESPONSE_CMD / READ_VAR_CMD / WRITE_VAR_CMD
  - build_mock_snet_monitor_payload test removed (function no longer in v2 codec)
"""
...
def test_build_frame_basic():
    frame = build_frame(0xC0, SnetCommand.IO_REQUEST, b'\x00')
...
def test_build_write_var_frame():
    ...
    view = decode_frame_view(frame)
    assert view.cmd == SnetCommand.WRITE_VAR
...
def test_build_read_var_frame():
    ...
    view = decode_frame_view(frame)
    assert view.cmd == SnetCommand.READ_VAR
